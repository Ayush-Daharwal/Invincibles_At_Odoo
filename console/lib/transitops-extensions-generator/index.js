 ...
 * 
 * This file is automatically generated by the Transitops extension build system.
 * Any manual changes will be overwritten on the next build.
 * 
 * @generated
 * @copyright © ${year} Transitops Pte Ltd. All rights reserved.
...
`;
    ...
    included(app) {
        ...
        console.log('\n' + '/'.repeat(70));
        console.log('[Transitops] Extension Build System');
    ...
    async generateExtensionFiles() {
        ...
        if (fs.existsSync(extensionsDir)) {
            console.log('[Transitops] Cleaning up old extensions directory...');
        ...
        if (extensions.length > 0) {
            console.log(`[Transitops] Discovered ${extensions.length} extension(s)`);
            extensions.forEach((ext) => {
                console.log(`[Transitops]   - ${ext.name} (v${ext.version})`);
            });
            console.log('');
            ...
            console.log('[Transitops] No extensions found — generating empty extension loader to satisfy module dependencies.');
        }

        // Always generate loaders, router, and manifest so that
        // @transitops/console/extensions (app/extensions/index.js) always exists
            ...
            return fg(['node_modules/*/package.json', 'node_modules/*/*/package.json'], { cwd })
                .then((results) => {
                    for (let i = 0; i < results.length; i++) {
                        ...
                        if (!packageData || !packageData.keywords || !packageData.keywords.includes('transitops-extension') || !packageData.keywords.includes('ember-engine')) {
                            continue;
                        }
                        ...
                        extensions.push(this.only(packageData, ['name', 'description', 'version', 'transitops', 'keywords', 'license', 'repository']));
                    }
    ...
    generateExtensionShims(extensions) {
        ...
        extensions.forEach((extension) => {
            ...
            const mountPath = extension.transitops?.route || this.getExtensionMountPath(extension.name);
            ...
            console.log(`[Transitops]   ✓ Generated app/extensions/${mountPath}.js`);
        });
    },
...
`;
        ...
        console.log(`[Transitops]   ✓ Generated app/extensions/index.js`);
    },

    generateRouter(extensions) {
        const consoleExtensions = extensions.filter((extension) => !extension.transitops || extension.transitops.mount !== 'root');
        const rootExtensions = extensions.filter((extension) => extension.transitops && extension.transitops.mount === 'root');
        ...
        console.log(`[Transitops]   ✓ Generated app/router.js`);
    },
    ...
    generateExtensionsManifest(extensions) {
        ...
        const manifest = extensions.map((ext) => ({
            name: ext.name,
            version: ext.version,
            route: ext.transitops?.route,
        }));

        const manifestFile = path.join(publicDir, 'extensions.json');
        fs.writeFileSync(manifestFile, JSON.stringify(manifest, null, 2), 'utf8');
        console.log(`[Transitops]   ✓ Generated public/extensions.json`);
    },
    ...
    watchExtensionFiles() {
        ...
        this.getExtensions().then((extensions) => {
            ...
            watcher.on('change', (filePath) => {
                console.log(`\n[Transitops] Extension file changed: ${path.basename(filePath)}`);
                console.log('[Transitops] Regenerating extension files...\n');
