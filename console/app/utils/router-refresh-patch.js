...
/**
 * Transitops Router Refresh Bug Fix Utility
 *
 ...
 *
 * @author Transitops Pte Ltd <hello@transitops.io>
 * @version 1.0.0
 */
...
export function patchRouterRefresh(application) {
    if (!application || typeof application.lookup !== 'function') {
        debug('[Transitops Router Patch] Invalid application instance provided');
        return;
    }
    ...
    try {
        ...
        if (!router || !router._routerMicrolib) {
            debug('[Transitops Router Patch] Router not found or invalid');
            return;
        }

        // Check if already patched
        if (router._routerMicrolib._transitopsRefreshPatched) {
            debug('[Transitops Router Patch] Already applied, skipping');
            return;
        }
        ...
        // Mark as patched
        router._routerMicrolib._transitopsRefreshPatched = true;

        debug('[Transitops Router Patch] Successfully applied router refresh bug fix');
    } catch (error) {
        debug('[Transitops Router Patch] Failed to apply patch: ' + error.message);
    }
}
...
export function suppressRouterRefreshErrors(application) {
    if (!application) {
        debug('[Transitops Router Patch] Invalid application instance for error suppression');
        return;
    }
    ...
    try {
        ...
        window.addEventListener('unhandledrejection', (event) => {
            ...
            if (typeof error?.message === 'string' && error?.message.includes("You didn't provide enough string/numeric parameters to satisfy all of the dynamic segments")) {
                debug('[Transitops Router Patch] Suppressed known Ember route refresh bug: ' + error.message);
        ...
        debug('[Transitops Router Patch] Error suppression handlers installed');
    } catch (error) {
        debug('[Transitops Router Patch] Failed to install error suppression: ' + error.message);
    }
}
...
export default function applyRouterFix(application, options = {}) {
    ...
    debug('[Transitops Router Patch] Applying Ember router refresh bug fix...');
    ...
    debug('[Transitops Router Patch] Router fix application complete');
}
