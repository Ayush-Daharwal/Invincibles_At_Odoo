import config from '@transitops/console/config/environment';
import toBoolean from '@transitops/ember-core/utils/to-boolean';
 ...
 */
const CACHE_KEY = 'transitops_runtime_config';
const CACHE_VERSION_KEY = 'transitops_runtime_config_version';
...
export default async function loadRuntimeConfig() {
    ...
    try {
        ...
        const response = await fetch('/transitops.config.json', {
            cache: 'default', // Use browser cache if available
        });

        if (!response.ok) {
            debug('[Runtime Config] No transitops.config.json found, using built-in config defaults');
            return;
        }
