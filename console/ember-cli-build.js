...
module.exports = function (defaults) {
    const app = new EmberApp(defaults, {
        ...
        fingerprint: {
            exclude: ['leaflet/', 'leaflet-images/', 'socketcluster-client.min.js', 'transitops.config.json', 'extensions.json'],
        },
    ...
    if (toBoolean(process.env.DISABLE_RUNTIME_CONFIG)) {
        runtimeConfigTree = writeFile('transitops.config.json', '{}');
    } else {
        runtimeConfigTree = new Funnel('.', {
            files: ['transitops.config.json'],
            destDir: '/',
        });
    }
