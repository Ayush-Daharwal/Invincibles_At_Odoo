...
module('Unit | Service | installation', function (hooks) {
    ...
    test('it detects Transitops not configured errors', function (assert) {
        const service = this.owner.lookup('service:installation');

        assert.true(service.isNotConfiguredError({ error: 'transitops_not_configured' }));
        assert.true(service.isNotConfiguredError({ errors: ['transitops_not_configured'] }));
        assert.true(service.isNotConfiguredError({ payload: { error: 'transitops_not_configured' } }));
        assert.true(service.isNotConfiguredError(new Error('transitops_not_configured')));
    ...
    test('it redirects to install when onboarding check reports unconfigured state', async function (assert) {
        ...
        class FetchStub extends Service {
            get() {
                return Promise.reject({ error: 'transitops_not_configured' });
            }
        }
    ...
    test('it stays on install route when Transitops is not configured', async function (assert) {
        class FetchStub extends Service {
            get() {
                return Promise.reject({ error: 'transitops_not_configured' });
            }
    ...
    test('it subscribes to install completion socket channel', async function (assert) {
        ...
        assert.strictEqual(subscribedChannel, 'transitops.install');
    ...
    test('it reloads on install complete event only', function (assert) {
        ...
        service.handleInstallCompleteEvent({ event: 'transitops.installed', installed: true });
