...
use Transitops\Exceptions\TransitopsRequestValidationException;
use Transitops\FleetOps\Events\EntityActivityChanged;
use Transitops\FleetOps\Events\EntityCompleted;
use Transitops\FleetOps\Events\OrderDispatchFailed;
use Transitops\FleetOps\Events\OrderStarted;
use Transitops\FleetOps\Events\WaypointActivityChanged;
use Transitops\FleetOps\Events\WaypointCompleted;
use Transitops\FleetOps\Exports\OrderExport;
use Transitops\FleetOps\Flow\Activity;
use Transitops\FleetOps\Http\Controllers\FleetOpsController;
use Transitops\FleetOps\Http\Requests\BulkDispatchRequest;
use Transitops\FleetOps\Http\Requests\CancelOrderRequest;
use Transitops\FleetOps\Http\Requests\Internal\CreateOrderRequest;
use Transitops\FleetOps\Http\Resources\v1\Index\Order as OrderIndexResource;
use Transitops\FleetOps\Http\Resources\v1\Order as OrderResource;
use Transitops\FleetOps\Http\Resources\v1\Proof as ProofResource;
use Transitops\FleetOps\Imports\OrdersImport;
use Transitops\FleetOps\Jobs\FinalizeInternalOrderCreation;
use Transitops\FleetOps\Jobs\NotifyBulkAssignedDriver;
use Transitops\FleetOps\Models\Driver;
use Transitops\FleetOps\Models\Entity;
use Transitops\FleetOps\Models\Order;
use Transitops\FleetOps\Models\OrderConfig;
use Transitops\FleetOps\Models\Payload;
use Transitops\FleetOps\Models\Place;
use Transitops\FleetOps\Models\Proof;
use Transitops\FleetOps\Models\ServiceQuote;
...
use Transitops\FleetOps\Support\ResolvesOrderServiceStops;
use Transitops\FleetOps\Support\Utils;
use Transitops\Http\Requests\ExportRequest;
use Transitops\Http\Requests\Internal\BulkActionRequest;
use Transitops\Models\File;
use Transitops\Models\Type;
use Transitops\Support\Auth;
use Transitops\Support\TemplateString;
...
class OrderController extends FleetOpsController
{
    ...
    public function createRecord(Request $request)
    {
            ...
            $record = $this->model->createRecordFromRequest(
                ...
                function ($request, &$input) {
                    ...
                    if ($serviceQuote instanceof ServiceQuote && $serviceQuote->fromIntegratedVendor()) {
                        // create order with integrated vendor, then resume transitops order creation
                        try {
                    ...
                    if (!$resolvedOrderConfig && $hasExplicitOrderConfig) {
                        throw new TransitopsRequestValidationException(['order_config_uuid' => 'The selected order config is invalid.']);
                    }
        ...
        } catch (TransitopsRequestValidationException $e) {
    ...
    /**
     * Process import files (excel,csv) into Transitops order data.
     *
     * @return \Illuminate\Http\Response
     */
     ...
     * Updates a order to canceled and updates order activity.
     *
     * @param \Transitops\Http\Requests\CancelOrderRequest $request
     *
     * @return \Illuminate\Http\Response
     */
    ...
    public function updateActivity(string $id, Request $request)
    {
        ...
        /**
         * @var \Transitops\LaravelMysqlSpatial\Types\Point
         */
     ...
     *
     * @return \Transitops\Http\Resources\v1\Order
     */
