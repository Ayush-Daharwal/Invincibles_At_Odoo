...
use Transitops\FleetOps\Http\Resources\v1\Orchestrator\Order as OrchestratorOrderResource;
use Transitops\FleetOps\Models\Contact;
use Transitops\FleetOps\Models\Driver;
use Transitops\FleetOps\Models\Manifest;
use Transitops\FleetOps\Models\ManifestStop;
...
use Transitops\FleetOps\Orchestration\Engines\DriverAssignmentEngine;
use Transitops\FleetOps\Orchestration\Engines\RouteSequencingEngine;
use Transitops\FleetOps\Orchestration\OrchestrationEngineRegistry;
        ...
        $configs = OrderConfig::where('company_uuid', $companyUuid)
            ...
            ->map(function ($config) {
                ...
                if ($customFields->isEmpty()) {
                    $customFields = \Transitops\Models\CustomField::where('subject_uuid', $config->uuid)
                        ->orderBy('order')
                        ->get();
                }
                ...
                if (!empty($firstRow['customer_email']) || !empty($firstRow['customer_phone']) || !empty($firstRow['customer_name'])) {
                    ...
                    if ($customerEntityType === 'vendor') {
                        ...
                        if ($vendor) {
                            ...
                            $customerType = 'Transitops\\FleetOps\\Models\\Vendor';
                        }
                    } else {
                        $contact = $this->resolveOrCreateContact($firstRow, $companyUuid, 'customer');
                        if ($contact) {
                            $customerUuid = $contact->uuid;
                            $customerType = 'Transitops\\FleetOps\\Models\\Contact';
                        }
                    }
                }
                ...
                if (!empty($firstRow['facilitator_email']) || !empty($firstRow['facilitator_phone']) || !empty($firstRow['facilitator_name'])) {
                    ...
                    if ($facilitatorEntityType === 'contact') {
                        ...
                        if ($contact) {
                            ...
                            $facilitatorType = 'Transitops\\FleetOps\\Models\\Contact';
                        }
                    } else {
                        $vendor = $this->resolveOrCreateVendor($firstRow, $companyUuid, 'facilitator');
                        if ($vendor) {
                            $facilitatorUuid = $vendor->uuid;
                            $facilitatorType = 'Transitops\\FleetOps\\Models\\Vendor';
                        }
                    }
                }
