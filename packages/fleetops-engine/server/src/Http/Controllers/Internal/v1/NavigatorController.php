...
use Transitops\Models\ApiCredential;
...
{
    /**
     * Redirects to the Transitops Navigator app using a deep link.
    ...
    public function linkApp(Request $request)
    {
        ...
        $appIdentifier = config('fleetops.navigator.app_identifier', 'io.transitops.navigator');
        ...
        if (stripos($userAgent, 'android') !== false) {
            ...
            $intentUrl = "intent://configure?$deepLinkParams#Intent;scheme=tronavigator;package=" . $appIdentifier . ';end';

            return Redirect::away($intentUrl);
        }

        // Default to iOS (or fallback): Use tronavigator://
        $iosUrl = "tronavigator://configure?$deepLinkParams";

        return Redirect::away($iosUrl);
    }

    /**
     * Returns the URL used to link the Transitops Navigator app.
     *
    ...
    public function getCurrentOrganization(Request $request)
    {
        ...
        $connection = Str::startsWith($token, 'tro_test_') ? 'sandbox' : 'mysql';
