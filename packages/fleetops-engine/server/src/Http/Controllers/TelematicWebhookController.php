...
use Transitops\FleetOps\Support\Telematics\TelematicProviderRegistry;
use Transitops\FleetOps\Support\Telematics\TelematicService;
use Transitops\Support\IdempotencyManager;
...
class TelematicWebhookController extends Controller
{
    ...
    public function handle(Request $request, string $providerKey): JsonResponse
    {
        ...
        // resolved by provider identifiers first; Transitops URL/header ids are
        // optional configuration affordances where provider dashboards allow it.
        $telematicId = $request->query('telematic') ?? $request->query('integration') ?? $request->header('X-Transitops-Telematic');
