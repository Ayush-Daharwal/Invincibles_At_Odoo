...
use Transitops\FleetOps\Scopes\DriverScope;
...
use Transitops\LaravelMysqlSpatial\Types\Point as SpatialPoint;
use Transitops\Models\File;
use Transitops\Models\Model;
use Transitops\Models\Schedule;
use Transitops\Models\ScheduleAvailability;
...
class Driver extends Model
{
    ...
    {
        // Explicit FK columns required — Transitops uses *_uuid not *_id
    ...
    public static function getAvatarOptions(): Collection
    {
        ...
        // Create default avatars included from transitops
        $avatars = collect($options)->mapWithKeys(
            function ($option) {
                $key = str_replace(['.svg', '.png'], '', $option);

                return [$key => Utils::assetFromTransitops('static/driver-icons/' . $option)];
            }
        );
