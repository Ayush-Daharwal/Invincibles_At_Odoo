...
class TrackingNumber extends Model
{
    ...
    /**
     * Generates a transitops tracking number.
     *
     * @var string
     */
    public static function generateTrackingNumber($region = 'SG', $length = 10): string
    {
        $company     = \Transitops\Models\Company::where('uuid', session('company'))->withoutGlobalScopes()->first();
        $companyName = $company ? strtoupper(substr($company->name, 0, 3)) : null;
        $number      = $companyName ?? 'TRO';
    ...
    /**
     * Generates a unique transitops tracking number.
     *
     * @var string
     */
