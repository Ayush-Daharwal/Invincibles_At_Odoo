...
use Transitops\FleetOps\Integrations\Lalamove\Lalamove;
use Transitops\FleetOps\Support\Payment;
use Transitops\FleetOps\Support\Utils;
use Transitops\Models\Model;
use Transitops\Traits\Expirable;
...
class ServiceQuote extends Model
{
    ...
    public function getStripeProduct(): ?\Stripe\Product
    {
        ...
        if ($stripe) {
            ...
            if (!$product) {
                $product = $stripe->products->create(['name' => $this->public_id, 'description' => 'Service Quote for ' . Utils::moneyFormat($this->amount, $this->currency), 'metadata' => ['transitops_id' => $this->public_id]]);
            }
