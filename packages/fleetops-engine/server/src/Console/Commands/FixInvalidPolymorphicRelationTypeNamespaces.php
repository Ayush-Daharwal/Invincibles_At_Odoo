...
namespace Transitops\FleetOps\Console\Commands;
...
class FixInvalidPolymorphicRelationTypeNamespaces extends Command
{
    ...
    {
        $models = [
            ['model' => \Transitops\FleetOps\Models\Order::class, 'columns' => ['customer_type', 'facilitator_type']],
            ['model' => \Transitops\FleetOps\Models\Place::class, 'columns' => ['owner_type']],
            ['model' => \Transitops\FleetOps\Models\Entity::class, 'columns' => ['customer_type']],
            ['model' => \Transitops\FleetOps\Models\PurchaseRate::class, 'columns' => ['customer_type']],
            ['model' => \Transitops\FleetOps\Models\Device::class, 'columns' => ['attachable_type']],
        ];
        ...
        foreach ($records as $record) {
            foreach ($columns as $column) {
                if (isset($record->$column)) {
                    ...
                    if (Str::startsWith($normalizedOriginal, 'Transitops\Models')) {
                        $newNamespace    = Str::replaceFirst('Transitops\Models', 'Transitops\FleetOps\Models', $normalizedOriginal);
