...
use Transitops\Support\Utils as TransitopsUtils;
...
class Utils extends TransitopsUtils
{
    /**
     ...
     *
     * @param \Transitops\FleetOps\Models\Place $place   the place object to retrieve the address from
     ...
     */
    public static function getAddressStringForPlace(\Transitops\FleetOps\Models\Place $place, $useHtml = false, $except = [])
    {
    ...
    public static function getPointFromMixed($coordinates): ?Point
    {
        ...
        if ($coordinates instanceof \Transitops\FleetOps\Models\Place) {
            $coordinates = $coordinates->location;
        }

        if ($coordinates instanceof \Transitops\FleetOps\Models\Driver) {
            $coordinates = $coordinates->location;
        }

        if ($coordinates instanceof \Transitops\FleetOps\Models\Vehicle) {
            $coordinates = $coordinates->location;
        }
        ...
        if ($coordinates instanceof \Transitops\LaravelMysqlSpatial\Eloquent\SpatialExpression) {
        ...
        if (is_string($coordinates)) {
            ...
            if (Str::startsWith($coordinates, 'place_')) {
                $resolvedPlace = \Transitops\FleetOps\Models\Place::where('public_id', $coordinates)->first();
                if ($resolvedPlace instanceof \Transitops\FleetOps\Models\Place) {
            ...
            if (Str::startsWith($coordinates, 'driver_')) {
                $resolvedDriver = \Transitops\FleetOps\Models\Driver::where('public_id', $coordinates)->first();
                if ($resolvedDriver instanceof \Transitops\FleetOps\Models\Driver) {
            ...
            if (Str::isUuid($coordinates)) {
                $resolvedPlace = \Transitops\FleetOps\Models\Place::where('uuid', $coordinates)->first();
                if ($resolvedPlace instanceof \Transitops\FleetOps\Models\Place) {
                    return static::getPointFromMixed($resolvedPlace);
                }

                $resolvedDriver = \Transitops\FleetOps\Models\Driver::where('uuid', $coordinates)->first();
                if ($resolvedDriver instanceof \Transitops\FleetOps\Models\Driver) {
                    return static::getPointFromMixed($resolvedDriver);
                }
     ...
     *
     * @param \Transitops\FleetOps\Models\Place|Point|array $origin
     * @param \Transitops\FleetOps\Models\Place|Point|array $destination
     */
     ...
     *
     * @return \Transitops\LaravelMysqlSpatial\Types\MultiPolygon|null the MultiPolygon object or null if not found
     */
    public static function createPolygonFromCountry(string $country): ?\Transitops\LaravelMysqlSpatial\Types\MultiPolygon
    {
        ...
        if ($feature) {
            return \Transitops\LaravelMysqlSpatial\Types\MultiPolygon::fromJson(json_encode($feature->geometry));
        }
     ...
     *
     * @param \Transitops\LaravelMysqlSpatial\Types\Polygon $polygon the Polygon object from which to calculate the centroid
     *
     * @return array returns the centroid of the polygon as an array of coordinates
     */
    public static function getPolygonCentroid(\Transitops\LaravelMysqlSpatial\Types\Polygon $polygon)
    {
     ...
     *
     * @param \Transitops\LaravelMysqlSpatial\Types\MultiPolygon $multiPolygon the MultiPolygon object from which to calculate the centroid
     *
     * @return array returns the centroid of the multi-polygon as an array of coordinates
     */
    public static function getMultiPolygonCentroid(\Transitops\LaravelMysqlSpatial\Types\MultiPolygon $multiPolygon)
    {
     ...
     *
     * @param \Transitops\LaravelMysqlSpatial\Types\Polygon|null $polygon the Polygon object from which to extract coordinates
     *
     ...
     */
    public static function getCoordinatesFromPolygon(?\Transitops\LaravelMysqlSpatial\Types\Polygon $polygon): array
    {
     ...
     */
    public static function getModelClassName($table, $namespaceSegments = '\\Transitops\\FleetOps\\'): string
    {
     ...
     *
     * @return \Transitops\LaravelMysqlSpatial\Eloquent\SpatialExpression|null returns a SpatialExpression object if the input is a valid GeoJSON, or null if the input is not valid
     *
     * @throws \InvalidArgumentException if the input is not a valid GeoJSON object
     */
    public static function createSpatialExpressionFromGeoJson($geoJson): ?\Transitops\LaravelMysqlSpatial\Eloquent\SpatialExpression
    {
        ...
        $geo  = \Transitops\LaravelMysqlSpatial\Types\Geometry::fromJson($json);

        // Return a new SpatialExpression object
        return new \Transitops\LaravelMysqlSpatial\Eloquent\SpatialExpression($geo);
    }
     ...
     * @throws \InvalidArgumentException if the input is not a valid GeoJSON object
     */
    public static function createGeometryObjectFromGeoJson($geoJson): ?\Transitops\LaravelMysqlSpatial\Types\Geometry
    {
        if (!static::isGeoJson($geoJson)) {
            return null;
        }
        ...
        return \Transitops\LaravelMysqlSpatial\Types\Geometry::fromJson($json);
    }
