...
|
*/
Route::prefix(config('fleetops.api.routing.prefix'))->namespace('Transitops\FleetOps\Http\Controllers')->group(
    function ($router) {
        /*
        |--------------------------------------------------------------------------
        ...
        */
        $router->group(['prefix' => 'v1', 'middleware' => ['transitops.api', Transitops\FleetOps\Http\Middleware\TransformLocationMiddleware::class], 'namespace' => 'Api\v1'], function ($router) {
            // drivers routes
            ...
            $router->group(['prefix' => 'customers', 'middleware' => []], function () use ($router) {
                ...
                $router->group(['middleware' => [Transitops\FleetOps\Http\Middleware\AuthenticateCustomerToken::class]], function () use ($router) {
            ...
            function ($router) {
                ...
                $router->group(
                    [
                        ...
                        'namespace'  => 'v1',
                        'middleware' => [
                            'transitops.protected',
                            Transitops\FleetOps\Http\Middleware\TransformLocationMiddleware::class,
                            Transitops\FleetOps\Http\Middleware\SetupDriverSession::class,
                        ],
                    ],
                    function ($router) {
                        $router->get('search', 'SearchController@search');

                        $router->transitopsRoutes(
                            'contacts',
                        ...
                        $router->transitopsRoutes('entities');
                        $router->transitopsRoutes(
                            'fleets',
                            function ($router, $controller) {
                        ...
                        $router->transitopsRoutes('fuel-provider-sync-runs');
                        $router->transitopsRoutes(
                        ...
                        $router->transitopsRoutes('order-configs');
                        $router->transitopsRoutes('payloads');
                        $router->transitopsRoutes(
                            'places',
                            function ($router, $controller) {
                        ...
                        $router->transitopsRoutes('proofs');
                        $router->transitopsRoutes('purchase-rates');
                        $router->transitopsRoutes('routes');
                        $router->transitopsRoutes('positions', function ($router, $controller) {
                        ...
                        $router->transitopsRoutes('tracking-numbers');
                        $router->transitopsRoutes('tracking-statuses');
                        $router->transitopsRoutes(
                            'vehicles',
                            function ($router, $controller) {
                                $router->get('statuses', $controller('statuses'));
                        ...
                        $router->transitopsRoutes('vehicle-devices');
                        $router->transitopsRoutes(
                            'vendors',
                            function ($router, $controller) {
                                $router->get('statuses', $controller('statuses'));
                        ...
                        $router->transitopsRoutes('devices', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                        });
                        $router->post('devices/{id}/attach', 'DeviceController@attach');
                        $router->post('devices/{id}/detach', 'DeviceController@detach');
                        $router->transitopsRoutes('device-events', function ($router, $controller) {
                            $router->post('{id}/mark-processed', $controller('markProcessed'));
                        });
                        $router->transitopsRoutes('sensors', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                        });
                        $router->transitopsRoutes('telematics', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->get('providers', $controller('providers'));
                        ...
                        $router->transitopsRoutes('maintenance-schedules', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        ...
                        $router->transitopsRoutes('work-orders', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                            $router->post('{id}/send', $controller('sendEmail'));
                        });
                        $router->transitopsRoutes('maintenances', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        ...
                        $router->transitopsRoutes('equipment', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        });
                        $router->transitopsRoutes('parts', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        });
                        $router->transitopsRoutes('warranties');
                        $router->group(
                            ['prefix' => 'query'],
                            function () use ($router) {
