...
module('Integration | Component | dashboard/widget-panel', function (hooks) {
    ...
    test('it can render widgets from an explicit widget source dashboard', async function (assert) {
        this.widgetService.catalogs = {
            dashboard: [{ id: 'transitops-blog', name: 'Transitops Blog', description: 'News', icon: 'newspaper', category: 'Core', default: true }],
        ...
        assert.dom('[data-widget-key="transitops-blog"]').doesNotExist('does not fall back to the default dashboard catalog when an explicit source is present');
    });
});
