...
import transitionEnd from '@transitops/ember-ui/utils/transition-end';
import { getDestinationElement } from '@transitops/ember-ui/utils/dom';
import usesTransition from '@transitops/ember-ui/utils/decorators/uses-transition';
import isFastBoot from '@transitops/ember-ui/utils/is-fastboot';
import deprecateSubclassing from '@transitops/ember-ui/utils/deprecate-subclassing';
...
/**
  ...
  <Modal @onSubmit={{action "submit"}} as |Modal|>
    <Modal.header>
      <h4 class="tro--modal-title"><i class="glyphicon glyphicon-alert"></i> Alert</h4>
    </Modal.header>
    <Modal.body>
...
export default class Modal extends Component {
     ...
     * 'center' will apply the `tro--modal-dialog-centered` class
     *
     * @property position
     * @type {string}
     * @default 'top'
     ...
     * 'true' will apply the `tro--modal-dialog-scrollable` class
     *
     * @property scrollable
     * @type boolean
     * @default false
     ...
     *  apply the `tro--modal-fullscreen` class when using `true` and
     *  `tro--modal-fullscreen-[x]-down` class when using BS breakpoints
     *   ([x] = `sm`, `md`, `lg`, `xl`, `xxl`).
     *
    ...
    /**
     * The DOM element of the `.tro--modal` element.
     *
    ...
    @action doSubmit() {
        ...
        let forms = this.modalElement.querySelectorAll('.tro--modal-body form');
        if (forms.length > 0) {
    ...
    addBodyClass() {
        ...
        if (this.modalsManager.modals.length === 1) {
            ...
            if (isFastBoot(this)) {
                ...
                if (!existingClasses.includes('tro--modal-open')) {
                    document.body.setAttribute('class', `tro--modal-open ${existingClasses}`);
                }
            } else {
                document.body.classList.add('tro--modal-open');
            }
        }
    }
    ...
    removeBodyClass() {
        ...
        if (this.modalsManager.modals.length === 0) {
            ...
            document.body.classList.remove('tro--modal-open');
        }
    }
    ...
    @computed('modalElement') get scrollbarWidth() {
        ...
        scrollDiv.className = 'tro--modal-scrollbar-measure';
