...
Transitops AI is an operations copilot and task automation module for Transitops. It adds a global AI prompt to the Transitops console, records each AI turn as an auditable task, and gives Transitops extensions a capability framework for exposing safe AI-readable context and previewable actions.

This extension was scaffolded with the Transitops CLI and contains both the Ember engine and Laravel backend package for `transitops/ai`.

## Current Status

This module is in active development. The current version includes:

- A global floating Transitops AI prompt opened from the header tray.
- Persistent chat sessions with history, continuation, ending, and soft-delete support.
- System-wide admin configuration for enabling Transitops AI and selecting a provider/model.
...
Install this extension like any Transitops extension package:

```bash
tro install transitops/ai --path /path/to/transitops
```

The `--path` value should point to the Transitops instance directory containing both `console/` and `api/`.

For local development inside a Transitops workspace, ensure the console workspace includes `@transitops/ai-engine`, then rebuild the console assets after frontend changes.

## Admin Configuration

Transitops AI is configured system-wide by admins. Provider keys are not stored per company.

In the console admin settings:

1. Open **AI > Provider Settings**.
2. Enable Transitops AI.
3. Select a provider.
...
When enabled, Transitops AI registers a compact magic-wand tray button in the Transitops console header. The same prompt can be opened globally with the configured keyboard shortcut.
...
Transitops AI does not give providers arbitrary database access. Modules expose AI functionality explicitly by registering capabilities.
