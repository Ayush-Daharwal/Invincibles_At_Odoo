...
use Transitops\Ai\Services\AnthropicProvider;
use Transitops\Ai\Services\LocalAIProvider;
use Transitops\Ai\Services\OpenAIProvider;
...
test('provider manager falls back to local when ai is disabled or provider is unknown', function () {
    ...
    expect($manager->providerNameFor(['enabled' => false, 'provider' => 'openai']))->toBe('local')
        ->and($manager->modelFor(['enabled' => false, 'provider' => 'openai']))->toBe('transitops-local-preview')
