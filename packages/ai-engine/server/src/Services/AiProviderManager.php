...
class AiProviderManager implements AIProviderInterface
{
        ...
        return [
            'providers' => [
                [
                    'label'         => 'Local Preview',
                    'value'         => 'local',
                    'default_model' => 'transitops-local-preview',
                    'models'        => [
                        ['label' => 'Transitops Local Preview', 'value' => 'transitops-local-preview'],
                    ],
                ],
                [
                    'label'         => 'OpenAI',
    ...
    {
        if (!Arr::get($config, 'enabled', false)) {
            return 'transitops-local-preview';
        }

        return Arr::get($this->normalizeConfig($config), 'default_model', 'transitops-local-preview');
    }
}
