...
class AnthropicProvider implements AIProviderInterface
{
    ...
    public function test(array $config = []): array
    {
        ...
        $response = Http::timeout(30)
            ...
            ->post($this->messagesUrl($providerConfig), [
                'model'      => $model,
                'system'     => 'You are testing Transitops AI provider connectivity.',
                'max_tokens' => 32,
                'messages'   => [
                    [
                        'role'    => 'user',
                        'content' => 'Reply with: Transitops AI provider test OK.',
                    ],
                ],
            ]);
    ...
    {
        return 'You are Transitops AI, an operations copilot inside Transitops. Answer concisely, focus on logistics and operational actions, use Transitops capability context as grounded system data, use Transitops temporal context for all relative dates, and never claim that an action has been executed unless Transitops provided a tool result confirming it.';
    }
    ...
    protected function userInstruction(AiTask $task, array $capabilityContext = []): string
    {
        ...
        return trim((string) $task->prompt) . "\n\nTransitops route context:\n" . $routeContext . "\n\nTransitops capability context:\n" . $aiContext;
    }
