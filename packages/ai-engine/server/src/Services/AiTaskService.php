...
namespace Transitops\Ai\Services;

use Transitops\Ai\Contracts\AIActionCapabilityInterface;
use Transitops\Ai\Contracts\AIProviderInterface;
use Transitops\Ai\Models\AiSession;
use Transitops\Ai\Models\AiTask;
use Transitops\Ai\Models\AiTaskStep;
...
class AiTaskService
{
    ...
    public function createFromRequest(Request $request): AiTask
    {
        ...
        $model       = $this->provider instanceof AiProviderManager ? $this->provider->modelFor($config) : 'transitops-local-preview';
        ...
        if (!empty($actionPreviews)) {
            ...
            $providerContext[] = [
                'capability'  => 'transitops.ai.action_previews',
                'type'        => 'action_preview',
                'data'        => $actionPreviews,
                'instruction' => 'A Transitops action preview has already been prepared from the user prompt. Do not ask again for details already present in the preview draft. Do not say the action has been applied until Transitops returns an apply result.',
            ];
        ...
        try {
            ...
            $task->update([
                ...
                'model'            => Arr::get($result, 'model', 'transitops-local-preview'),
    ...
    protected function sessionContext(AiTask $task): ?array
    {
        ...
        return [
            'capability'  => 'transitops.ai.session_context',
            'type'        => 'session_context',
            'instruction' => 'Use this recent Transitops AI chat history as conversation context. Do not repeat questions already answered by the user in earlier turns.',
            'data'        => [
