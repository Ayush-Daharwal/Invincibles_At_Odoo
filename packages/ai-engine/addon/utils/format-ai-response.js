...
function renderTable(rows, alignments) {
    ...
    return `<div class="transitops-ai-table-wrapper"><table><thead><tr>${headerHtml}</tr></thead><tbody>${bodyHtml}</tbody></table></div>`;
}
...
function alignmentAttribute(alignment) {
    return alignment ? ` class="transitops-ai-table-align-${alignment}"` : '';
}
