...
namespace Transitops\RegistryBridge\Support;

use Transitops\Support\Utils as SupportUtils;
...
class Utils extends SupportUtils
{
     ...
     * It sets the registry for npm and the Transitops scope registry.
    ...
    public static function setConsoleNpmrcConfig(bool $reset = false): bool
    {
        ...
        $config                = implode(PHP_EOL, [
            ...
            '@transitops:registry=' . rtrim($registryHost, '/') . '/',
        ]) . PHP_EOL;
    ...
    public static function setComposerAuthConfig(): bool
    {
        ...
        $registryHost     = static::getDomainFromUrl(config('registry-bridge.registry.host', env('REGISTRY_HOST', 'https://registry.transitops.io')), true);
        $registryToken    = config('registry-bridge.registry.token', env('REGISTRY_TOKEN'));
