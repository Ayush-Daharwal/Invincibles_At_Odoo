...
use Transitops\RegistryBridge\Exceptions\InstallFailedException;
...
class RegistryExtensionBundle extends Model
{
    ...
    protected static function extractBundleFile(File $bundle, $filenames = 'extension.json', $options = []): ?\stdClass
    {
        ...
        $tempDir         = sys_get_temp_dir() . '/' . str_replace(['.', ','], '_', uniqid('transitops_archive_', true));
        mkdir($tempDir);
    ...
    public function installComposerPackage(): void
    {
        ...
        $process->setWorkingDirectory('/transitops/api');
    ...
    public function installEnginePackage(): void
    {
        ...
        $process->setWorkingDirectory(config('transitops.console.path'));
        $process->setTimeout(3600 * 2);
    ...
    /**
     * Initiates and manages the build process of the Transitops console using PNPM.
     *
     * This method prepares and executes the build process for the Transitops console. It monitors the build
