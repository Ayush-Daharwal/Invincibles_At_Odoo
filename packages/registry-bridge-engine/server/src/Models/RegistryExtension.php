...
namespace Transitops\RegistryBridge\Models;
...
class RegistryExtension extends Model
{
    ...
    public function getIconUrlAttribute(): ?string
    {
        ...
        return data_get($this, 'icon.url', 'https://tro-assets.s3.ap-southeast-1.amazonaws.com/static/default-extension-icon.svg');
    }
     ...
     * `uuid`, `public_id`, and `slug`. If the package name starts with 'transitops/', it also attempts to match the slug extracted from the package name.
     *
    ...
    public static function lookup(string $packageName): ?RegistryExtension
    {
        return static::where('status', 'published')->where(function ($query) use ($packageName) {
            ...
            // Check for transitops/ prefix and match slug
            if (Str::startsWith($packageName, 'transitops/')) {
    ...
    public function getStripeProduct(): ?\Stripe\Product
    {
        ...
        if ($stripe) {
            ...
            if (!$product) {
                $product = $stripe->products->create(['name' => $this->name, 'description' => $this->description, 'metadata' => ['transitops_id' => $this->public_id]]);
            }
    ...
    public function createStripeCheckoutSession(string $returnUri): \Stripe\Checkout\Session
    {
        ...
        // Calculate the fee transitops takes for faciliation of extension
