...
use Transitops\RegistryBridge\Models\RegistryExtensionPurchase;
...
class RegistryPaymentsController extends Controller
{
     ...
     *
     * This method utilizes the Transitops utility class to create a Stripe Express account and saves the
    ...
    public function getStripeCheckoutSessionStatus(Request $request)
    {
        ...
        if (session('company')) {
            ...
            $purchaserType = 'Transitops\\Models\\Company';
        }
        // Check if authenticated via bearer token (developer account)
        elseif ($request->bearerToken()) {
            $registryUser = \Transitops\RegistryBridge\Models\RegistryUser::where('token', $request->bearerToken())->first();
            if ($registryUser && $registryUser->developer_account_uuid) {
                $purchaserUuid = $registryUser->developer_account_uuid;
                $purchaserType = 'Transitops\\RegistryBridge\\Models\\RegistryDeveloperAccount';
            }
        }
            ...
            if (isset($session->status) && $session->status === 'complete') {
                RegistryExtensionPurchase::firstOrCreate(
                    ...
                    [
                        ...
                        'company_uuid'               => $purchaserType === 'Transitops\\Models\\Company' ? $purchaserUuid : null,
                    ]
                );
            }
    ...
    public function getAuthorReceivedPayments(Request $request)
    {
        ...
        return TransitopsResource::collection($payments)->additional(['total_amount' => $totalPurchaseAmount]);
    }
