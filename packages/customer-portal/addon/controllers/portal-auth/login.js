...
import pathToRoute from '@transitops/ember-core/utils/path-to-route';
...
export default class PortalAuthLoginController extends Controller {
    ...
    @action async login(event) {
        ...
        try {
            ...
            await this.session.authenticate('authenticator:transitops', { identity, password }, rememberMe, 'auth/login', { namespace: CUSTOMER_PORTAL_NAMESPACE });
        } catch (error) {
            this.failedAttempts++;
    ...
    @action forgotPassword() {
        return this.hostRouter.transitionTo('customer-portal.portal-auth.forgot-password').then(() => {
            if (this.email) {
