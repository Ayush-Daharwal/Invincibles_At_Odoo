<h1 align="center">Transitops Customer Portal</h1>

<p align="center">
    An extendable customer workspace for Transitops with self-service orders, billing, documents, support, account settings, and dashboard widgets.
</p>

<p align="center">
    <a href="https://github.com/transitops/customer-portal/blob/main/LICENSE.md"><img src="https://img.shields.io/badge/license-AGPL--3.0--or--later-blue.svg" alt="License"></a>
    <a href="https://www.npmjs.com/package/@transitops/customer-portal-engine"><img src="https://img.shields.io/npm/v/@transitops/customer-portal-engine.svg" alt="npm package"></a>
    <a href="https://packagist.org/packages/transitops/customer-portal-api"><img src="https://img.shields.io/packagist/v/transitops/customer-portal-api.svg" alt="Packagist package"></a>
...
<p align="center">
    <img src="https://www.transitops.io/images/screenshots/customer-portal/customer-portal-dashboard.webp" alt="Transitops Customer Portal" width="960">
</p>

## Overview

Transitops Customer Portal is a first-party Transitops extension that gives customers a secure, self-service workspace connected to Transitops Console, FleetOps, and Ledger. It ships as an Ember engine for the frontend and a Laravel package for the backend API.
...
- **Extension points** - Sidebar and login registries, dashboard/widget registration, virtual routes, and menu integration for extending the portal from other Transitops packages.
...
app/        Re-exports that make addon modules available to the consuming Transitops Console app
...
The Ember engine mounts at the Transitops route configured in `package.json`:
...
```bash
transitops/customer-portal-api
```
...
Install the backend package in the Transitops API:

```bash
composer require transitops/customer-portal-api
```

Install the Ember engine in Transitops Console:

```bash
pnpm install @transitops/customer-portal-engine
```

The package also depends on Transitops shared frontend packages and first-party backend packages, including Core API, FleetOps, and Ledger.

## Development Setup

For local Transitops extension development, use Transitops's package linker so Console and API resolve this checkout instead of published package versions. See the official [Transitops Development Setup guide](https://www.transitops.io/docs/platform/quickstart/development-setup) for the full workflow.

Clone Transitops with submodules:

```bash
git clone https://github.com/transitops/transitops.git
cd transitops
git submodule update --init --recursive
```

Install the package linker once from the Transitops repository root:

```bash
npm link
tro-package-linker --help
```

Enable the customer portal package:

```bash
tro-package-linker enable customer-portal
tro-package-linker install customer-portal
```

You can let the linker run install commands immediately:

```bash
tro-package-linker enable customer-portal --install
```
...
Transitops runs Laravel Octane, so reload the application worker after PHP changes:

```bash
docker compose exec application php artisan octane:reload
```

For frontend work, run the Transitops Console development server locally or in Docker. The local path is usually the fastest:
...
Portal admins can configure customer portal behavior from Transitops Console settings. The package includes API support for:
...
When developing inside the Transitops monorepo, prefer `tro-package-linker` over manual `package.json`, `composer.json`, or workspace edits so local development links remain reversible.

## License

Transitops Customer Portal is released under the GNU Affero General Public License v3.0 or later. See [LICENSE.md](LICENSE.md) for details.
