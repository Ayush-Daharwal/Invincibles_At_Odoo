...
Route::prefix(config('customer-portal.api.routing.prefix', 'customer-portal'))->namespace('Transitops\CustomerPortal\Http\Controllers')->group(
    function ($router) {
        /*
        |--------------------------------------------------------------------------
        ...
        $router->prefix(config('customer-portal.api.routing.internal_prefix', 'int'))->namespace('Internal')->group(
            function ($router) {
                $router->group(
                    ...
                    function ($router) {
                        $router->group(
                            ['prefix' => 'auth', 'middleware' => [Transitops\Http\Middleware\ThrottleRequests::class]],
                            function ($router) {
                                $router->post('login', 'AuthController@login');
                            }
                        );
                        $router->group(
                            ['prefix' => 'two-fa', 'middleware' => [Transitops\Http\Middleware\ThrottleRequests::class]],
                            function ($router) {
