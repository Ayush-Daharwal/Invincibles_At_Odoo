...
use Transitops\Http\Requests\TwoFaValidationRequest;
use Transitops\Models\User;
use Transitops\Support\TwoFactorAuth;
...
class TwoFaController extends Controller
{
        ...
        try {
            ...
            if (!$tokenOwner instanceof User || $tokenOwner->type !== 'customer') {
                ...
                if ($tokenOwner instanceof User) {
                    return response()->error('Console users must sign in through the Transitops console.', 403, ['code' => 'console_login_not_allowed']);
                }
    ...
    protected function resolveCustomerByIdentity(?string $identity): User
    {
        ...
        if (!$user || $user->type !== 'customer') {
            if ($user) {
                throw new HttpResponseException(response()->error('Console users must sign in through the Transitops console.', 403, ['code' => 'console_login_not_allowed']));
            }
