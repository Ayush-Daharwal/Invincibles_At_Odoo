...
use Transitops\CustomerPortal\Services\PortalOrderService;
use Transitops\FleetOps\Http\Resources\v1\Order as OrderResource;
use Transitops\FleetOps\Http\Resources\v1\OrderConfig as OrderConfigResource;
...
use Transitops\FleetOps\Models\PurchaseRate;
...
use Transitops\Models\Transaction;
use Transitops\Models\TransactionItem;
...
class OrderController extends Controller
{
    ...
    {
        if (
            !class_exists(\Transitops\Ledger\Models\Invoice::class)
            || !class_exists(\Transitops\Ledger\Models\Transaction::class)
            || !class_exists(\Transitops\Ledger\Services\InvoiceService::class)
        ) {
            return;
        }
        ...
        try {
            ...
            if (!$invoice) {
                $invoice = app(\Transitops\Ledger\Services\InvoiceService::class)->createFromOrder($order, [
            ...
            if ($reference) {
                ...
                app(\Transitops\Ledger\Services\InvoiceService::class)->send($invoice);
            }
        } catch (\Throwable $e) {
    ...
    protected function findLedgerInvoice(Order $order, PurchaseRate $purchaseRate): ?\Transitops\Ledger\Models\Invoice
    {
        $invoice = \Transitops\Ledger\Models\Invoice::where('company_uuid', $order->company_uuid)
    ...
    protected function recordLedgerPaymentForPurchaseRate($invoice, PurchaseRate $purchaseRate, string $reference, ?string $checkoutSessionId, ?string $paymentIntentId): void
    {
        ...
        $paymentExists = \Transitops\Ledger\Models\Transaction::where('company_uuid', $invoice->company_uuid)
            ->where('subject_uuid', $invoice->uuid)
            ->where('subject_type', \Transitops\Ledger\Models\Invoice::class)
        ...
        app(\Transitops\Ledger\Services\InvoiceService::class)->recordPayment($invoice, $amount, [
