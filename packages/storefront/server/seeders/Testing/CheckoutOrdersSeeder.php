...
namespace Transitops\Storefront\Seeders\Testing;
...
use Transitops\Storefront\Seeders\Testing\Concerns\SeedsTestingData;
...
class CheckoutOrdersSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $store = $this->seededModel(Store::class, 'store:transitops-market');
        if (!$store) {
    ...
    public function purgeSeedData(): void
    {
        ...
        $this->deleteFrom($this->transitopsConnection(), 'transaction_items', fn ($query) => $query->whereIn('transaction_uuid', $transactionUuids)->orWhere('meta->seed', static::SEED_NAME));
    ...
    {
        if (!Schema::connection($this->transitopsConnection())->hasTable('ledger_journals')) {
            return;
        }

        DB::connection($this->transitopsConnection())
