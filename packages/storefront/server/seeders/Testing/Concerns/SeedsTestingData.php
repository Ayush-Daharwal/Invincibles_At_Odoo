...
namespace Transitops\Storefront\Seeders\Testing\Concerns;

use Transitops\Models\Company;
use Transitops\Models\CompanyUser;
use Transitops\Models\Model as TransitopsModel;
...
trait SeedsTestingData
{
    ...
    protected function prepareCompany(): ?Company
    {
        ...
        if (!$company) {
            $this->command?->error('No company found. Create a Transitops company before running Storefront testing seeders.');
    ...
    {
        /** @var TransitopsModel|Model $model */
        $model      = new $modelClass();
        $table      = $model->getTable();
        $connection = $model->getConnectionName();
    ...
    protected function transitopsConnection(): string
    {
        return config('transitops.connection.db');
    }
}
