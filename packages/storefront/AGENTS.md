...
Storefront extension for logistics-first commerce, order intake, storefront configuration, products, checkout flows, and commerce-to-logistics workflows.

## Allowed Work

- Bug fixes for storefront configuration, product, checkout, order intake, and commerce logistics workflows.
- Small backend, Ember engine, test, and documentation updates that follow existing Storefront patterns.
- API validation, resource, and schema updates when paired with documentation/spec checks.

## Restricted Work

- Large rewrites of checkout, order intake, or marketplace flows without explicit approval.
- Gateway, payment, or customer data handling changes without human review.
- API behavior changes without checking `transitops/postman` and `transitops/transitops.io`.
...
Some changes require documentation updates in `transitops/transitops.io`.

Some API-related changes additionally require updates to API specification files in `transitops/postman`.
...
If API behavior changes, the agent must determine whether corresponding specification updates are needed in `transitops/postman`.
...
If API specification files are updated in `transitops/postman`, the agent should also determine whether a documentation PR or regeneration step is required in `transitops/transitops.io`.
...
3. reference the expected documentation area in `transitops/transitops.io`;
4. reference any required specification updates in `transitops/postman`.
