<p align="center">
    <img src="https://tro-assets.s3.ap-southeast-1.amazonaws.com/static/storefront-logo.svg" alt="Transitops Storefront" width="380">
</p>

<p align="center">
    Logistics-first headless commerce and marketplace infrastructure for Transitops.
</p>

<p align="center">
    <a href="https://www.transitops.io/docs/storefront">Documentation</a>
    |
    <a href="https://github.com/transitops/storefront">GitHub</a>
    |
    <a href="https://www.transitops.io/platform/storefront">Platform Overview</a>
    |
    <a href="https://www.transitops.io">Transitops</a>
</p>
...
<p align="center">
    <img src="https://www.transitops.io/images/screenshots/storefront/storefront-dashboard.webp" alt="Transitops Storefront dashboard" width="860">
</p>

## Overview

Storefront is the commerce extension for Transitops. It combines a Laravel API package with an Ember engine for the Transitops Console, giving operators the tools to manage stores, products, carts, checkout, customers, marketplace networks, and fulfillment workflows from one logistics-native system.

Unlike a generic e-commerce plugin, Storefront is built around the handoff from purchase to delivery. A checkout can create Fleet-Ops orders, attach storefront order metadata, expose customer and commerce details in the Transitops Console, and keep operators close to the real delivery lifecycle.

Read the official guide at [transitops.io/docs/storefront](https://www.transitops.io/docs/storefront).
...
| Area | Path | Purpose |
| --- | --- | --- |
| Ember engine | `addon/`, `app/`, `config/`, `translations/` | Transitops Console UI, routes, models, components, services, widgets, and translations. |
...
The Ember package is published as `@transitops/storefront-engine`. The Laravel package is published as `transitops/storefront-api`.
...
The protected internal API is mounted under `storefront/int/v1`. It powers the Transitops Console and includes:
...
Storefront is designed to run inside a Transitops installation with `transitops/core-api` and `transitops/fleetops-api` available.
...
composer require transitops/fleetops-api
composer require transitops/storefront-api
```

### Frontend Package

```bash
pnpm install @transitops/storefront-engine
```

## Development

For the full local Transitops workflow, see the [Transitops Development Setup guide](https://www.transitops.io/docs/platform/quickstart/development-setup). That guide covers cloning the main repository with submodules, mounting live package source, linking extensions, running the Console dev server, and reloading the API after backend changes.

### Use Local Storefront Source in Transitops

When developing Storefront inside the Transitops monorepo, run the package linker from the repository root so the local `packages/storefront` source replaces the published Storefront packages used by Console and API:

```bash
tro-package-linker enable storefront
tro-package-linker install storefront
```
...
If the linker is not installed yet, install it once from the Transitops repository root:
...
For frontend changes, run the Transitops Console dev server as described in the development setup guide; linked Ember packages are watched and live-reloaded by the dev server.
...
For customer app configuration, see the [Storefront App configuration docs](https://www.transitops.io/docs/storefront/app/configuration).

## Documentation

- [Storefront documentation](https://www.transitops.io/docs/storefront)
- [Products and catalog](https://docs.transitops.io/guides/storefront/products/)
- [Orders and checkout](https://www.transitops.io/docs/storefront/orders/overview)
- [Checkout flow](https://www.transitops.io/docs/storefront/orders/checkout)
- [Store locations](https://www.transitops.io/docs/storefront/stores/store-locations)
- [Storefront App configuration](https://www.transitops.io/docs/storefront/app/configuration)

## Contributing

Storefront follows the same review expectations as the rest of Transitops:
...
Transitops Storefront is open-source software licensed under the [AGPL-3.0-or-later](LICENSE.md).
