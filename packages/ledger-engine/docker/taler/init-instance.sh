...
ADMIN_TOKEN="${TALER_MERCHANT_ADMIN_TOKEN:-secret-token:transitops-taler-admin-dev}"
INSTANCE_ID="${TALER_MERCHANT_INSTANCE_ID:-default}"
INSTANCE_PASSWORD="${TALER_MERCHANT_INSTANCE_PASSWORD:-transitops-taler-dev-password}"
TOKEN_FILE="${TALER_MERCHANT_TOKEN_FILE:-/var/lib/transitops-ledger/taler/generated-token.txt}"
WEBHOOK_URL="${TALER_MERCHANT_WEBHOOK_URL:-http://httpd/ledger/webhooks/taler}"
PAYTO_URI="${TALER_MERCHANT_PAYTO_URI:-payto://x-taler-bank/taler-bank.lvh.me/default?receiver-name=Transitops%20Ledger}"
...
instance_payload="$(jq -n \
  --arg id "${INSTANCE_ID}" \
  --arg name "Transitops Ledger Local Test Merchant" \
  --arg website "${PUBLIC_BASE_URL}" \
  --arg password "${INSTANCE_PASSWORD}" \
  '{
    id: $id,
    name: $name,
    email: "dev@transitops.io",
    website: $website,
    auth: {
      method: "token",
      password: $password
    ...
    address: {
      ...
      address_lines: ["Transitops Ledger Taler test merchant"]
    },
    jurisdiction: {
      country: "US",
      town: "Local Development",
...
token_payload="$(jq -n '{scope: "all", description: "Transitops Ledger local development"}')"
...
webhook_payload="$(jq -n \
  --arg url "${WEBHOOK_URL}" \
  '{
    webhook_id: "transitops-ledger-pay",
    event_type: "pay",
    url: $url,
    http_method: "POST",
...
if [ "${webhook_code}" != "204" ] && [ "${webhook_code}" != "409" ]; then
  echo "Unable to register Transitops Ledger Taler webhook (HTTP ${webhook_code})." >&2
  printf '%s\n' "${webhook_response}" >&2
  exit 1
fi
