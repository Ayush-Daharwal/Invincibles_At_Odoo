...
CONFIG_FILE="${TALER_CONFIG_FILE:-/var/lib/transitops-ledger/taler/taler.conf}"
...
EXCHANGE_BANK_PASSWORD="${TALER_EXCHANGE_BANK_PASSWORD:-transitops-exchange-bank-password}"
MERCHANT_BANK_USER="${TALER_MERCHANT_BANK_USER:-default}"
MERCHANT_BANK_PASSWORD="${TALER_MERCHANT_BANK_PASSWORD:-transitops-merchant-bank-password}"
EXCHANGE_PAYTO_URI="${TALER_EXCHANGE_PAYTO_URI:-payto://x-taler-bank/taler-bank.lvh.me/exchange?receiver-name=Transitops%20Local%20Exchange}"
MERCHANT_PAYTO_URI="${TALER_MERCHANT_PAYTO_URI:-payto://x-taler-bank/taler-bank.lvh.me/default?receiver-name=Transitops%20Ledger}"
...
install -d -m 0755 \
  "$(dirname "${CONFIG_FILE}")" \
  /var/lib/transitops-ledger/taler/data/exchange/offline \
  /var/lib/transitops-ledger/taler/data/exchange/revocations \
  /var/lib/transitops-ledger/taler/run/secmod-eddsa \
  /var/lib/transitops-ledger/taler/run/secmod-rsa \
  /var/lib/transitops-ledger/taler/run/secmod-cs \
  /var/lib/transitops-ledger/taler/tmp
...
printf '%s\n' "${master_pub}" > /var/lib/transitops-ledger/taler/local-exchange-master-public-key.txt
...
libeufin-bank create-account \
  ...
  --name "Transitops Local Exchange" \
  --exchange \
  --public \
