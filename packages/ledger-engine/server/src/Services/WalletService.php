...
namespace Transitops\Ledger\Services;

use Transitops\Ledger\DTO\PurchaseRequest;
use Transitops\Ledger\Models\Account;
...
class WalletService
{
    ...
    public function deposit(
        ...
        return DB::transaction(function () use ($wallet, $amount, $description, $type, $options) {
            ...
            $transaction = Transaction::create([
                ...
                'settled_currency'       => $wallet->currency,
                'description'            => $desc,
    ...
    public function transfer(
        ...
        return DB::transaction(function () use ($fromWallet, $toWallet, $amount, $description, $options) {
            ...
            $fromTransaction = Transaction::create([
                ...
                'settled_at'        => now(),
                'settled_amount'    => $amount,
                'settled_currency'  => $fromWallet->currency,
                'description'       => $desc,
                'reference'         => $reference,
            ...
            $toTransaction = Transaction::create([
                ...
                'settled_at'        => now(),
                'settled_amount'    => $amount,
                'settled_currency'  => $toWallet->currency,
                'description'       => $desc,
                'reference'         => $reference,
                'meta'              => array_merge($options['meta'] ?? [], [
    ...
    public function topUp(
        ...
        if ($gatewayResponse->isSuccessful() && $gatewayResponse->status === \Transitops\Ledger\DTO\GatewayResponse::STATUS_SUCCEEDED) {
            // Find the persisted GatewayTransaction
            $gatewayTransaction = \Transitops\Ledger\Models\GatewayTransaction::where(
     ...
     *
     * @return \Illuminate\Support\Collection<\Transitops\Ledger\Models\Wallet>
     */
