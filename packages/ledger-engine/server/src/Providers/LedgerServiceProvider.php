...
namespace Transitops\Ledger\Providers;

use Transitops\Ledger\Events\PaymentFailed;
use Transitops\Ledger\Events\PaymentSucceeded;
use Transitops\Ledger\Events\RefundProcessed;
use Transitops\Ledger\Listeners\HandleFailedPayment;
use Transitops\Ledger\Listeners\HandleProcessedRefund;
use Transitops\Ledger\Listeners\HandleSuccessfulPayment;
use Transitops\Ledger\PaymentGatewayManager;
use Transitops\Ledger\Services\InvoiceService;
use Transitops\Ledger\Services\LedgerService;
use Transitops\Ledger\Services\PaymentService;
use Transitops\Ledger\Services\RevenueLifecycleService;
use Transitops\Ledger\Services\WalletService;
use Transitops\Providers\CoreServiceProvider;
use Transitops\Services\TemplateRenderService;
use Illuminate\Support\Facades\Event;

if (!class_exists(CoreServiceProvider::class)) {
    throw new \Exception('Ledger cannot be loaded without `transitops/core-api` installed!');
}
...
class LedgerServiceProvider extends CoreServiceProvider
{
     ...
     */
    public $observers = [
        \Transitops\Ledger\Models\Invoice::class => \Transitops\Ledger\Observers\InvoiceObserver::class,
        \Transitops\Models\Company::class        => \Transitops\Ledger\Observers\CompanyObserver::class,
        \Transitops\Models\User::class           => \Transitops\Ledger\Observers\UserObserver::class,
        ...
        'Transitops\\FleetOps\\Models\\PurchaseRate' => \Transitops\Ledger\Observers\PurchaseRateObserver::class,
        'Transitops\\FleetOps\\Models\\Order'        => \Transitops\Ledger\Observers\OrderAccountingObserver::class,
    ];
    ...
    public function boot()
    {
        ...
        if ($this->app->runningInConsole()) {
            $this->commands([
                \Transitops\Ledger\Console\Commands\ProvisionLedgerDefaults::class,
                \Transitops\Ledger\Console\Commands\BackfillTransactionDirection::class,
                \Transitops\Ledger\Console\Commands\UpdateOverdueInvoices::class,
                \Transitops\Ledger\Console\Commands\RepairRevenueLifecycle::class,
            ]);
        }
    }
    ...
    private function registerInvoiceTemplateContext(): void
    {
        ...
        TemplateRenderService::registerContextType('invoice', [
            ...
            'model'       => \Transitops\Ledger\Models\Invoice::class,
            'variables'   => [
                // ── Invoice ──────────────────────────────────────────────────
