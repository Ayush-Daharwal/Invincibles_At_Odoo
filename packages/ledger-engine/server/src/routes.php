...
| and are protected by the 'transitops.protected' middleware which requires a
| valid Transitops session or API key.
|
*/
Route::prefix(config('ledger.api.routing.prefix', 'ledger'))->namespace('Transitops\Ledger\Http\Controllers')->group(
    function ($router) {
        /*
        |--------------------------------------------------------------------------
        ...
        */
        ...
        /*
        |--------------------------------------------------------------------------
        | Internal Routes (Authenticated via Transitops Session — Console)
        |--------------------------------------------------------------------------
        */
        $router->prefix(config('ledger.api.routing.internal_prefix', 'int'))->namespace('Internal')->group(
            function ($router) {
                $router->prefix(config('ledger.api.routing.version_prefix', 'v1'))->namespace('v1')->group(
                    function ($router) {
                        $router->group(
                            ['middleware' => ['transitops.protected']],
                            function ($router) {
                                // ----------------------------------------------------------------
                                // Chart of Accounts
                                // ----------------------------------------------------------------
                                ...
                                // ----------------------------------------------------------------
                                $router->transitopsRoutes('transactions');

                                // ----------------------------------------------------------------
                                // Payment Gateways
                                // ----------------------------------------------------------------
                                // The 'drivers' static route must be registered BEFORE transitopsRoutes
