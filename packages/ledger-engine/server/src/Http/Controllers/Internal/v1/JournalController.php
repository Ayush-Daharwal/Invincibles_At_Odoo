...
use Transitops\Ledger\Http\Resources\v1\Journal as JournalResource;
use Transitops\Ledger\Models\Journal;
...
class JournalController extends LedgerResourceController
{
    ...
    public function createManual(Request $request): JsonResponse
    {
        ...
        $debitAccount = \Transitops\Ledger\Models\Account::where('company_uuid', session('company'))
            ->where('uuid', $request->input('debit_account_uuid'))
            ->firstOrFail();

        $creditAccount = \Transitops\Ledger\Models\Account::where('company_uuid', session('company'))
