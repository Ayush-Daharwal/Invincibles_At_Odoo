...
export default class ResourceActionService extends Service {
    ...
    #getImportTemplate() {
        const templateModelName = this.modelName
            ...
            .join('_');
        const baseAssetUrl = this.baseAssetUrl ?? 'https://tro-assets.s3.ap-southeast-1.amazonaws.com';
        const importTemplatePath = this.importTemplatePath ?? 'import-templates';
        const importTemplateName = this.importTemplateName ?? `Transitops_${templateModelName}_Import_Template.xlsx`;
