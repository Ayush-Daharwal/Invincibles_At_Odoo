...
/**
 * Normalize geoiplookup.io response to match Transitops whois format
 */
...
/**
 * Normalize ipapi.co response to match Transitops whois format
 */
...
function getCachedWhois() {
    try {
        const cached = localStorage.getItem('transitops:whois');
        if (!cached) {
            return null;
        }
        ...
        if (age > maxAge) {
            localStorage.removeItem('transitops:whois');
            return null;
        }
...
function cacheWhois(data) {
    try {
        localStorage.setItem('transitops:whois', JSON.stringify(data));
    } catch (error) {
